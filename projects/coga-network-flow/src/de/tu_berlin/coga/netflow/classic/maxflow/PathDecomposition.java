...
public class PathDecomposition {
    ...
    private static boolean DFS(AbstractNetwork network, Node x, List<Node> sinks, StaticFlowPath path){             
    	visited.set(x, true);
    ...
    private static boolean pathEndsOnSink(AbstractNetwork network, List<Node> sinks, StaticFlowPath path){
        if (!path.edges().empty()){
            for (Node t : sinks){
...
/*    private static ArrayList<Node> getNodesOnPath(StaticFlowPath path){
    ...
    private static void updateCapacities(AbstractNetwork network, StaticFlowPath path, IdentifiableIntegerMapping<Edge> flow){
