 ...
 */
public abstract class PushRelabel extends AbstractAlgorithm<MaximumFlowProblem, MaximumFlow> {
    /** The number of nodes. */
    protected int n;
    /** The number of edges. */
    protected int m;
    ...
    /**
     * Performs the first phase of a maximum flow computation. Sends as much flow to the sink as possible. Afterwards,
     * the value of a maximum flow is known, but the nodes may have a flow excess.
     */
    protected abstract void computeMaxFlow();

    /**
     * After the first part of the algorithm, that is computed by {@link #computeMaxFlow()}, the flow is not feasible as
     * several nodes may have a flow excess. This method reduces the excess by sending the flow back to the source.
     * Thus, the result is a feasible maximum flow.
     */
    ...
    /**
     * Decides whether a push is applicable to a certain edge. That is, if the node is active and is admissible at the
     * same time.
     *
    ...
    /**
     * Decides whether relabel is applicable to a certain node. That is, if the node is active (has positive excess) but
     * for none of the outgoing edges push is applicable.
     *
...
//
    ...
    /**
     * Decides whether an edge is admissible. That is, if it has positive residual capacity and the label of the start
     * vertex is by one higher than the label of the end vertex.
     *
     * @param e the edge which is tested
    ...
    /**
     * Decides whether a node is active. That is, if it has positive excess. The source and the sink cannot be active.
     *
     * @param v the node which is tested
