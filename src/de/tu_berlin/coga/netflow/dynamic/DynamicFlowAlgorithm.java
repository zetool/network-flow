...
public abstract class DynamicFlowAlgorithm<U extends DynamicFlowProblem> extends Algorithm<U, FlowOverTime> {
  ...
  /**
   * Returns the calculated flow as {@code PathBasedFlowOverTime} object. Returns null if the algorithm has not run yet,
   * but throws an exception if it has run and only computed an edge based flow.
